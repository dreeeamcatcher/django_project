...
class Cart(models.Model):
    ...
    def change_qty(self, qty, item_id):
        ...
        cart_item.item_total = int(qty) * decimal.Decimal(cart_item.product.price)
        cart_item.save()
        new_cart_total = 0.00
        for item in cart.items.all():
            new_cart_total += float(item.item_total)
        cart.cart_total = decimal.Decimal(new_cart_total)
        cart.save()
...
class Order(models.Model):
    ...
    type = models.CharField(max_length=40, choices=(('Самовывоз', 'Самовывоз'), ('Доставка', 'Доставка')), default='Самовывоз')
